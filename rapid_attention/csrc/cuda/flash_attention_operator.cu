...
template <int BBits = 3, int MBase = 0, int SShift = 3> struct CuteSwizzle {
...
struct GSMemLdstConfig {
  ...
  static constexpr index_t gmem_thr_offset(int tid) {
    return GMemStride::crd2idx(tid_to_thr_row(tid), tid_to_thr_col(tid), 0, 0);
  }
...
struct SRMemLdstConfig {
  ...
  lane_to_thr_swizzle_stride_s2rmem(int lane_id) {
    if constexpr (std::is_same_v<Swizzle, NoSwizzle>) {
      ...
      int s1 = 32 * binary_to_pm1(
                        (base_swizzle_offset & (base_offset_cmp << 1)) == 0);
      int s2 = 16 * binary_to_pm1((base_swizzle_offset & base_offset_cmp) == 0);
  ...
  lane_to_thr_swizzle_stride_r2smem(int lane_id) {
    if constexpr (std::is_same_v<Swizzle, NoSwizzle>) {
      ...
      int s1 = 32 * binary_to_pm1(
                        (base_swizzle_offset & (base_offset_cmp << 2)) == 0);
      int s2 = 16 * binary_to_pm1(
                        (base_swizzle_offset & (base_offset_cmp << 1)) == 0);
      int s3 = 8 * binary_to_pm1((base_swizzle_offset & base_offset_cmp) == 0);
...
copy_warp_fragment_SM2RF(RmemType &rmem, value_t *smem,
  ...
  static_assert(Cfg::OpIters::rows() == RmemType::ViewType2x2::Shape::rows() /
                ...
                "Cfg::OpRmemStride.row");
  static_assert(RmemType::Shape::cols() == 1, "RmemType::Shape.cols must be 2");
...
copy_warp_fragment_transposed_SM2RF(RmemType &rmem, value_t *smem,
  ...
  for (int ss = 0; ss < Cfg::OpIters::swizzle_spaces(); ++ss) {
    ...
    for (int ic = 0; ic < Cfg::OpIters::tiles(); ++ic) {
      ...
      int rr = ss * Cfg::SmemShape::tiles() + ic * Cfg::OpRmemStride::row();
  ...
  for (int ss = 0; ss < Cfg::OpIters::swizzle_spaces(); ++ss) {
    ...
    for (int it = 0; it < Cfg::OpIters::tiles(); ++it) {
      ...
      for (int ir = 0; ir < Cfg::OpIters::rows(); ++ir) {
        ...
        reinterpret_cast<uint32_t *>(&smem[smem_idx])[0] = rmem_uint(
            ir * Cfg::OpRmemStride::row(),
            it * Cfg::OpRmemStride::col() + ss * Cfg::SmemShape::tiles());
      }
    }
  }
}
...
template <const FlashForwardKernelConfig &cfg> constexpr bool valid_config() {
...
template <FlashForwardKernelConfig _CFG> struct StaticForwardKernelConfig {
  ...
  using SmemSwizzle = std::conditional_t<CFG.swizzled, SmemSwizzle_, NoSwizzle>;
  ...
  using GSMemLdstConfigQO =
      GSMemLdstConfig<SmemSwizzle, OpGSMemStride, GSMemShapeQO, GSSMemQOStride,
                      GMemStride>;
  using GSMemLdstConfigKV =
      GSMemLdstConfig<SmemSwizzle, OpGSMemStride, GSMemShapeKV, GSSMemKVStride,
                      GMemStride>;
  ...
  using S2RSmemStrideV =
      SMemStride<SwizzleTileSize, 1, SRMemTileSize * SwizzleTileSize,
                 GSSMemKVStride::swizzle_space()>;
  using RmemShapeV = RmemShape<SRMemFragmentsDHead / 1, SRMemTileFragments / 2,
                               SRMemTilesB_c, 1, 2>;
  ...
  using RmemConfigQ =
      RmemLdstConfig<RmemShapeQ, N::Q_rmem_tile_buffer_size,
                     CFG.Q_mma_load_K_fragments == 0, false /*RowMajorOpTile*/>;
  using GSRConfigQ = GSRMemLdstConfig<RmemConfigQ, false, N::QO_rows_per_warp,
                                      false /*compute_over_entire_block*/
                                      >;
  using Q_t =
      GSRBlockTensor<GSRConfigQ, value_t, GSMemLdstConfigQO, S2RMemLdstConfigQ>;

  using RmemConfigK =
      RmemLdstConfig<RmemShapeK, N::K_rmem_tile_buffer_size,
                     CFG.K_mma_load_K_fragments == 0, true /*RowMajorOpTile*/>;
  using GSRConfigK =
      GSRMemLdstConfig<RmemConfigK, false, N::KV_ldst_rows_per_warp,
                       true /*compute_over_entire_block*/
                       >;
  using K_t =
      GSRBlockTensor<GSRConfigK, value_t, GSMemLdstConfigKV, S2RMemLdstConfigK>;

  using RmemConfigV =
      RmemLdstConfig<RmemShapeV, N::V_rmem_tile_buffer_size,
                     CFG.V_mma_load_K_fragments == 0, true /*RowMajorOpTile*/>;
  using GSRConfigV =
      GSRMemLdstConfig<RmemConfigV, true, N::KV_ldst_rows_per_warp,
                       true /*compute_over_entire_block*/
                       >;
  using V_t =
      GSRBlockTensor<GSRConfigV, value_t, GSMemLdstConfigKV, S2RMemLdstConfigV>;
  ...
  using GSRConfigO = GSRMemLdstConfig<RmemConfigO, false, N::QO_rows_per_warp,
                                      ...
                                      >;
  using O_t =
      GSRBlockTensor<GSRConfigO, value_t, GSMemLdstConfigQO, R2SMemLdstConfigO>;
...
std::map<FlashForwardKernelConfig, forward_kernel_fn> forward_kernels = {
    /*
            ...
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 0, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 0, false, true}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 0, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 0, true, false}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 0, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 0, true, true}>>},
            // (FP16, 128, 64, 32, 4): async+eager+swizzled+load_2_2_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 2, false, false}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 2, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 2, false, true}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 2, true, false}>>},
            // (FP16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 2, true, true}>>},
            // (FP16, 128, 64, 64, 4): async+load_0_0_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    false, false, 0, 0, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, false, false, 0, 0, 0, false, false}>>},
            // (FP16, 128, 64, 64, 4): async+swizzled+load_0_0_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    false, true, 0, 0, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, false, true, 0, 0, 0, false, false}>>},
            // (FP16, 128, 64, 64, 4): async+eager+swizzled+load_0_0_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 0, false, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 0, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 0, false, true}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_0_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 0, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 0, true, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_0_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 0, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 0, true, true}>>},
            // (FP16, 128, 64, 64, 4): async+eager+swizzled+load_0_0_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 2, false, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_2_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 2, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 2, false, true}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 2, true, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_0_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 2, true, true}>>},
            // (FP16, 128, 64, 64, 4): async+eager+swizzled+load_0_2_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 0, false, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 0, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 0, false, true}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_0_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 0, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 0, true, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_0_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 0, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 0, true, true}>>},
            // (FP16, 128, 64, 64, 4): async+eager+swizzled+load_0_2_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 2, false, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_2_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 2, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 2, false, true}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 2, true, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_0_2_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 0, 2, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 0, 2, 2, true, true}>>},
            // (FP16, 128, 64, 64, 4): async+eager+swizzled+load_2_2_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 2, 2, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 2, 2, 2, false, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 2, 2, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 2, 2, 2, true, false}>>},
            // (FP16, 128, 64, 64, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 64, 64, 4, true,
    true, true, 2, 2, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 64, 64, 4, true, true, true, 2, 2, 2, true, true}>>},
            // (FP16, 128, 128, 32, 4): async+eager+swizzled+load_2_2_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 0, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 0, false, true}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 0, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 0, true, false}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 0, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 0, true, true}>>},
            // (FP16, 128, 128, 32, 4): async+eager+swizzled+load_2_2_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 2, false, false}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 2, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 2, false, true}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 2, true, false}>>},
            // (FP16, 128, 128, 32, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 2, true, true}>>},
            // (FP16, 128, 128, 64, 4): async+eager+swizzled+load_2_2_0_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 0, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 0, false, true}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 0, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 0, true, false}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_0_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 0, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 0, true, true}>>},
            // (FP16, 128, 128, 64, 4): async+eager+swizzled+load_2_2_2_tiles
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 2, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 2, false, false}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_2_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 2, false, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 2, false, true}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 2, true, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 2, true, false}>>},
            // (FP16, 128, 128, 64, 4):
    async+eager+swizzled+load_2_2_2_tiles+buffer+opt_softmax
            {FlashForwardKernelConfig{torch::kFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 2, true, true},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 2, true, true}>>},
            // (BF16, 128, 64, 32, 4): async+eager+swizzled+load_2_2_0_tiles
            {FlashForwardKernelConfig{torch::kBFloat16, 128, 64, 32, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kBFloat16,
    128, 64, 32, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (BF16, 128, 64, 32, 4):
    async+eager+swizzled+load_2_2_0_tiles+opt_softmax
            {FlashForwardKernelConfig{torch::kBFloat16, 128, 64, 32, 4, true,
    ...
    128, 64, 32, 4, true, true, true, 2, 2, 2, true, true}>>},
            ...
            {FlashForwardKernelConfig{torch::kBFloat16, 128, 64, 64, 4, true,
    true, true, 0, 0, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kBFloat16,
    128, 64, 64, 4, true, true, true, 0, 0, 0, false, false}>>},
            // (BF16, 128, 64, 64, 4):
    ...
    128, 64, 64, 4, true, true, true, 0, 2, 2, true, true}>>},
            ...
            {FlashForwardKernelConfig{torch::kBFloat16, 128, 128, 32, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kBFloat16,
    128, 128, 32, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (BF16, 128, 128, 32, 4):
    ...
    128, 128, 32, 4, true, true, true, 2, 2, 2, true, true}>>},
            ...
            {FlashForwardKernelConfig{torch::kBFloat16, 128, 128, 64, 4, true,
    true, true, 2, 2, 0, false, false},
    &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{torch::kBFloat16,
    128, 128, 64, 4, true, true, true, 2, 2, 0, false, false}>>},
            // (BF16, 128, 128, 64, 4):
    ...
    // (BF16, 128, 128, 64, 4):
    // async+eager+swizzled+load_2_2_2_tiles+buffer+opt_softmax
    {FlashForwardKernelConfig{torch::kBFloat16, 128, 128, 64, 4, true, true,
                              true, 2, 2, 2, true, true},
     &flash_forward_kernel<StaticForwardKernelConfig<FlashForwardKernelConfig{
         torch::kBFloat16, 128, 128, 64, 4, true, true, true, 2, 2, 2, true,
         true}>>}};
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &rapid_flash_attention::flash_attention_forward, py::arg("kernel_cfg"),
