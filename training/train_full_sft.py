...
def main():
    ...
    # 构建训练数据集和数据加载器。
    train_ds = SFTDataset(
        GCTX.train_config.sft_train_data_path, trainer_ctx.tokenizer, max_len=trainer_ctx.lm_config.max_seq_len
    )
    train_sampler = None
    ...
    for epoch in range(start_epoch, GCTX.train_config.epochs):
        ...
        for step, (input_ids, labels) in enumerate(train_loader, start=start_step + 1):
            ...
            last_step = step
            lr = get_lr(epoch * iter_per_epoch + step, GCTX.train_config.epochs * iter_per_epoch, GCTX.train_config.learning_rate)
            ...
            if step % GCTX.train_config.log_interval == 0 or step == iter_per_epoch:
                ...
                eta_min = spend_time / max(step - last_step, 1) * (iter_per_epoch - step) // 60
                logger(f"Epoch [{epoch+1}/{GCTX.train_config.epochs}] Step [{step}/{iter_per_epoch}] "
            ...
            if (step % GCTX.train_config.save_interval == 0 or step == iter_per_epoch):
                ...
                torch.save({k: v.half().cpu() for k, v in state_dict.items()}, tmp_ckp_pth)
